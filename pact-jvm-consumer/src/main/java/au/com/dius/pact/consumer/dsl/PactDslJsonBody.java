 ...
 */
public class PactDslJsonBody extends PactDslJsonWithId<PactDslJsonBody> implements PactDslBody<PactDslJsonBody> {
   ...
   */
  public PactDslJsonBody(String rootPath, String rootName, PactDslJsonWithId parent) {
   ...
   */
  public PactDslJsonBody(String rootPath, String rootName, PactDslJsonWithId parent, PactDslJsonBody body) {
    super(parent, rootPath, rootName);
    this.body = body.body;
    this.matchers = body.getMatchers().copyWithUpdatedMatcherRootPrefix(rootPath);
  ...
  protected void putObject(DslPart object) {
    ...
    generators.addGenerators(object.getGenerators());
    String elementBase = StringUtils.difference(this.rootPath, object.getRootPath());
    if (StringUtils.isNotEmpty(object.getRootName())) {
      body.put(object.getRootName(), object.getBody());
    } else {
  ...
  protected void putArray(DslPart object) {
    ...
    if (StringUtils.isNotEmpty(object.getRootName())) {
      ...
      body.put(StringUtils.difference(this.rootPath, object.getRootPath()), object.getBody());
    }
  }
  ...
  public PactDslJsonWithId closeArray() {
    if (parent instanceof PactDslJsonArray) {
      closeChild();
      return ((PactDslJsonArray) parent).closeChild();
    } else {
  ...
  public PactDslJsonBody eachLike(String name, int numberExamples) {
    addRule(matcherKey(name), matchMin(0));
  ...
  public PactDslJsonBody minArrayLike(String name, Integer size, int numberExamples) {
    if (numberExamples < size) {
      throw new IllegalArgumentException(format("Number of example %d is less than the minimum size of %d",
          numberExamples, size));
    }
    addRule(matcherKey(name), matchMin(size));
    PactDslJsonArray parent = new PactDslJsonArray(matcherKey(name), "", this, true);
    parent.setNumberExamples(numberExamples);
    return new PactDslJsonBody(".", "", parent);
  }
  ...
  public PactDslJsonBody maxArrayLike(String name, Integer size, int numberExamples) {
    if (numberExamples > size) {
      throw new IllegalArgumentException(format("Number of example %d is more than the maximum size of %d",
          numberExamples, size));
    }
    addRule(matcherKey(name), matchMax(size));
    PactDslJsonArray parent = new PactDslJsonArray(matcherKey(name), "", this, true);
    parent.setNumberExamples(numberExamples);
    return new PactDslJsonBody(".", "", parent);
  }
  ...
  public PactDslJsonBody eachKeyMappedToAnArrayLike(String exampleKey) {
    if (FeatureToggles.isFeatureSet(Feature.UseMatchValuesMatcher)) {
      addRule(rootPath.endsWith(".") ? rootPath.substring(0, rootPath.length() - 1) : rootPath, ValuesMatcher.INSTANCE);
    } else {
      addRule(rootPath + "*", matchMin(0));
    }
  ...
  public PactDslJsonBody eachKeyLike(String exampleKey) {
    if (FeatureToggles.isFeatureSet(Feature.UseMatchValuesMatcher)) {
      ...
      addRule(rootPath + "*", TypeMatcher.INSTANCE);
    }
  ...
  public PactDslJsonBody eachKeyLike(String exampleKey, PactDslJsonRootValue value) {
    ...
    for (String matcherName : value.getMatchers().getMatchingRules().keySet()) {
      getMatchers().addRules(rootPath + "*" + matcherName, value.getMatchers().getMatchingRules().get(matcherName).getRules());
    }
    return this;
  }
  ...
  public PactDslJsonBody minMaxArrayLike(String name, Integer minSize, Integer maxSize, int numberExamples) {
    ...
    addRule(matcherKey(name), matchMinMax(minSize, maxSize));
    PactDslJsonArray parent = new PactDslJsonArray(matcherKey(name), "", this, true);
    parent.setNumberExamples(numberExamples);
    return new PactDslJsonBody(".", "", parent);
  }
  ...
  private void validateMinAndMaxAndExamples(Integer minSize, Integer maxSize, int numberExamples) {
    if (minSize > maxSize) {
      throw new IllegalArgumentException(format("The minimum size %d is more than the maximum size of %d",
          minSize, maxSize));
  ...
  @Override
  public PactDslJsonBody objectValue(String name, Object value) {
    if (value == null) {
  ...
  public PactDslJsonBody stringType(String name) {
    addGenerator(Category.BODY, matcherKey(name), new RandomStringGenerator(20));
  ...
  public PactDslJsonBody numberType(String name, Number number) {
    ...
    addRule(matcherKey(name), new NumberTypeMatcher(NumberTypeMatcher.NumberType.NUMBER));
    return this;
  }
  ...
  public PactDslJsonBody integerType(String name, Long number) {
    numberValue(name, number);
    addRule(matcherKey(name), new NumberTypeMatcher(NumberTypeMatcher.NumberType.INTEGER));
    return this;
  }
  ...
  public PactDslJsonBody decimalType(String name) {
    addGenerator(Category.BODY, matcherKey(name), new RandomDecimalGenerator(10));
  ...
  public PactDslJsonBody decimalType(String name, BigDecimal number) {
    numberValue(name, number);
    addRule(matcherKey(name), new NumberTypeMatcher(NumberTypeMatcher.NumberType.DECIMAL));
    return this;
  }
  ...
  public PactDslJsonBody stringMatcher(String name, String regex, String value) {
    if (!value.matches(regex)) {
      throw new InvalidMatcherException(format(INVALID_REGULAR_EXPRESSION, value, regex));
    }
  ...
  public PactDslJsonBody stringMatcher(String name, String regex) {
    addGenerator(Category.BODY, matcherKey(name), new RegexGenerator(regex));
  ...
  public PactDslJsonBody ipAddress(String name) {
    ...
    addRule(matcherKey(name), regexp(IP_ADDRESS));
    return this;
  }
  ...
  @Override
  public PactDslJsonBody id(Long id) {
    return id("id", id);
  }
